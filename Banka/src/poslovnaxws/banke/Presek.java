...
public class Presek implements Identifiable{
	 ...
	 * 
	 * @return possible object is {@link Presek.Zaglavlje }
	 * 
	 */
	 ...
	 * 
	 * @param value
	 *            allowed object is {@link Presek.Zaglavlje }
	 * 
	 */
	 ...
	 * 
	 * @return possible object is {@link Presek.StavkePreseka }
	 * 
	 */
	...
	/**
	 * Sets the value of the stavkePreseka property. Automatski se menjaju
	 * stavke.
	 * 
	 * @param value
	 *            allowed object is {@link Presek.StavkePreseka }
	 * 
	 */
	public void setStavkePreseka(Presek.StavkePreseka value) {
		this.stavkePreseka = value;
		if (zaglavlje == null)
			zaglavlje = new Zaglavlje(value);
		else{
			for (TStavkaPreseka stavka : value.getStavka())
	 ...
	 */
	@XmlAccessorType(XmlAccessType.FIELD)
	@XmlType(name = "", propOrder = { "brojRacuna", "datumNaloga",
			"brojPreseka", "prethodnoStanje", "brojPromenaUKorist",
			"ukupnoUKorist", "brojPromenaNaTeret", "ukupnoNaTeret",
			"novoStanje" })
	public static class Zaglavlje {
		...
		public Zaglavlje(StavkePreseka stavkePreseka) {
			List<TStavkaPreseka> stavke = stavkePreseka.getStavka();
			brojPreseka = new BigInteger(String.valueOf(stavke.size()));

			for (TStavkaPreseka stavka : stavke) {
		...
		/**
		 * Dodaje podatke iz stavke u zaglavlje.
		 * 
		 * @param stavka
		 */
		public void updateZaglavlje(TStavkaPreseka stavka) {
			double novoStanje = this.prethodnoStanje.doubleValue();
			prethodnoStanje = new BigDecimal(String.valueOf(novoStanje));

			if (stavka.getSmer().equals("T")) {
				double ukupno = ukupnoNaTeret.doubleValue();
				ukupno += stavka.getIznos().doubleValue();
				ukupnoNaTeret = new BigDecimal(String.valueOf(ukupno));
				brojPromenaNaTeret++;
				novoStanje -= stavka.getIznos().doubleValue();
				this.novoStanje = new BigDecimal(String.valueOf(novoStanje));
			} else if (stavka.getSmer().equals("K")) {
				double ukupno = ukupnoUKorist.doubleValue();
				ukupno += stavka.getIznos().doubleValue();
				ukupnoUKorist = new BigDecimal(String.valueOf(ukupno));
				brojPromenaUKorist++;
				novoStanje += stavka.getIznos().doubleValue();
				this.novoStanje = new BigDecimal(String.valueOf(novoStanje));
			}
	...
	/**
	 * Dodaje stavku u listu stavki. Automatski azurira zaglavlje.
	 * 
	 * @param stavka
	 */
	public void addStavka(TStavkaPreseka stavka) {
		stavkePreseka.getStavka().add(stavka);
		if (zaglavlje != null)
			zaglavlje.updateZaglavlje(stavka);
